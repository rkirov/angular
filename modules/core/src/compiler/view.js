...
export class View {
  ...
  proto: ProtoView;
  context: Object;
  _localContext: ContextWithVariableBindings;
  ...
  _createLocalContext() {
    ...
    for (var [ctxName, tmplName] of MapWrapper.iterable(this.proto.variableBindings)) {
      MapWrapper.set(locals, tmplName, null);
    }
    this._localContext = new ContextWithVariableBindings(null, locals);
  }
  ...
  setLocal(contextName: string, value) {
    ...
    this._localContext.set(templateName, value);
  }
  ...
  _hydrateContext(newContext) {
    if (isPresent(this._localContext)) {
      this._localContext.parent = newContext;
      newContext = this._localContext;
    }
  ...
  _dehydrateContext() {
    if (isPresent(this._localContext)) {
      this._localContext.parent = null;
      this._localContext.clearValues();
    }
    this.context = null;
  }
   ...
   * the view tree, without incurring the cost of recreating the object and the
   * underlying injectors and watch records.
   *
   * A dehydrated view has the following properties:
   *
   * - all injections are empty.
